...
void PathFinder::setArea(uint32 mapId, float x, float y, float z, uint32 area, float range)
{
    ...
    dtNavMesh const* cnavMesh = mmap->GetNavMesh(mapId, m_defaultInstanceId);
...
void PathFinder::ComputePathToRandomPoint(Vector3 const& startPoint, float maxRange)
{
    ...
    if (fail && canShortcut && m_sourceUnit->GetMap()->IsInLineOfSight(currPos.x, currPos.y, currPos.z + m_sourceUnit->GetCollisionHeight(), endPoint.x, endPoint.y, endPoint.z + m_sourceUnit->GetCollisionHeight(), m_sourceUnit->GetPhaseMask(), false))
    {
        BuildShortcut();
    }
}
